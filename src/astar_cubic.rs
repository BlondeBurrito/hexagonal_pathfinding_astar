...
pub fn astar_path(
	...
	if start_node.0.abs() > count_rings || start_node.1.abs() > count_rings || start_node.2.abs() > count_rings {
	...
	while queue[0].0 != end_node {
		...
		let available_nodes =
			node_neighbours_cubic(current_path.0, count_rings);
...
fn calculate_node_weight(
	...
	node_distance(current_node.clone(), end_node.clone()) as f32
}
...
mod tests {
	...
	fn astar_tick() {
		...
		nodes.insert((1, 0, -1), 15.0);
		nodes.insert((1, -1, 0), 14.0);
		nodes.insert((0, -1, 1), 2.0);
		nodes.insert((-1, 0, 1), 6.0);
		nodes.insert((-1, 1, 0), 7.0);
		nodes.insert((0, 2, -2), 1.0);
		nodes.insert((1, 1, -2), 14.0);
		nodes.insert((2, 0, -2), 1.0);
		nodes.insert((2, -1, -1), 1.0);
		nodes.insert((2, -2, 0), 1.0);
		nodes.insert((1, -2, 1), 1.0);
		nodes.insert((0, -2, 2), 1.0);
		nodes.insert((-1, -1, 2), 3.0);
		nodes.insert((-2, 0, 2), 1.0);
		nodes.insert((-2, 1, 1), 8.0);
		nodes.insert((-2, 2, 0), 1.0);
		nodes.insert((-1, 2, -1), 2.0);
		let end_node: (i32, i32, i32) = (2, 0, -2);
		let rings = 2;
		let best = astar_path(
			start_node,
			nodes,
		...
		let actual = vec![(0, 0, 0), (0, -1, 1), (1, -2, 1), (2, -2, 0), (2, -1, -1), (2, 0, -2)];
